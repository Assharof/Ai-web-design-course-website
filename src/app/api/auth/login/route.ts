...
import { createSession, ensureDemoUser, toSafeUser, verifyPassword } from "@/lib/auth";
...
export async function POST(request: Request) {
  try {
    ...
    if (!profile || !(await verifyPassword(password, profile.passwordHash))) {
      return Response.json({ error: "That email or password doesn’t match our records." }, { status: 401 });
    }
